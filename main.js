...
function startClock() {
  ...
  $("#clock").html(
    "<span>" + days[cTime.getDay()] + "</span>" + h + ":" + m + ":" + s
  );
  setTimeout(function() {
    startClock();
  }, 500);
    ...
    if (i.slice(4) == "") {
      $("#result").append(
        "[" +
          ...
          "@" +
          machine +
          ' ~]$ goto<br>Invalid command usage! Use "Goto [address]"'
      );
    } else {
    ...
    if (i.slice(4) == "") {
      $("#result").append(
        "[" +
          ...
          "@" +
          machine +
          ' ~]$ name<br>Invalid command usage! Use"Name [new name]"'
      );
    } else {
    ...
    if (i.slice(7) == "") {
      $("#result").append(
        "[" +
          ...
          "@" +
          machine +
          ' ~]$ machine<br>Invalid command usage! Use"Machine [new machine name]"'
      );
    } else {
      ...
      $("#result").append(
        "[" +
          ...
          "@" +
          machine +
          ' ~]$ name<br>Machine name changed to "' +
          machine +
          '"'
      );
    }
    ...
    if (i.slice("6") == "") {
      $("#result").append(
        "[" +
          ...
          "@" +
          machine +
          ' ~]$ search<br>Invalid command usage! Use"Search [search query]"'
      );
    } else {
      window.location.href =
        "http://google.com/search?q=" + i.slice(7).toLowerCase();
    }
  } else {
    $("#result").append(
      "[" +
        ...
        " ~]$ " +
        i.toLowerCase() +
        '<br>Invalid command! Use "Help" for a list of commands'
    );
  }
  if (i != "CLEAR") {
    $("#result").append("<br>");
